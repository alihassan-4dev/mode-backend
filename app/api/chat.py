...
async def create_chat_message(
    ...
    try:
        ...
        session_with_messages = await chat_store.get_session(db, user_id=user.id, session_id=session.id)
        ...
        await db.commit()
        session_after_reply = await chat_store.get_session(db, user_id=user.id, session_id=session.id)
        ...
        await db.rollback()
        logger.exception("Could not process chat message for user_id=%s", user.id, exc_info=exc)
        raise HTTPException(
